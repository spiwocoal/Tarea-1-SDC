...
suficientemente alta. Esto sucede porque el controladoe proporcional ajusta la 
salida de acuerdo con la magnitud del error actual.
Es decir el controlador proporcianal no alcanza la referencia deseada porque 
genera una accion correctiva proporcional al error, pero no puede eliminarlo 
completamente sin introducir componentes adicionales como la integral que 
veremos en  la siguiente pregunta.

Observamos que para el $\nara{k_{c}}$ encontrado, hace que  la salida  y las
 variables de estado, oscilan significativamente 
Esto es debido a que, el sistema al ser marginalmente estable en lugar de 
estable,en estado estacionario no convergerá nunca a un valor fijo, pero puesto 
que el
